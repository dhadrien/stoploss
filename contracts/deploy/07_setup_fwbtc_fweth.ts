...
const func: DeployFunction = async function (bre: BuidlerRuntimeEnvironment) {
  ...
  const FWBTC = await ethers.getContract("FWBTC");
  ...
  console.log(`
----------
07 SETUP FWBTC/FWETH
----------
  `);
  // Contract needed
  logStep("DEPLOYING UNISWAP V2 PAIR FWBTC/FWET");
  let pairAddress = await uniFactory.getPair(FWBTC.address, FWETH.address);
  if (pairAddress === NULL_ADDRESS) {
    await (await uniFactory.createPair(FWBTC.address, FWETH.address)).wait();
    pairAddress = await uniFactory.getPair(FWBTC.address, FWETH.address);
  ...
  save("UniPairFWBTCFWETH", {abi: uniPairArtifact.abi, address: pairAddress});
  ...
  if (reserveBefore1 === reserveBefore2 && reserveBefore2 === "0") {
    ...
    await (await FWBTC.approve(uniRouter.address, INIT_TOKEN_SUPPLY)).wait();
    await (
  ...
  const FWBTCReserve = await FWBTC.balanceOf(pairAddress);
  const FWETHREserve = await FWETH.balanceOf(pairAddress);
  console.log("Reserve for FWBTC: ", weiAmountToString(FWBTCReserve));
  console.log("Reserve for FWETH: ", weiAmountToString(FWETHREserve));

  logStep("DEPLOYING STOPLOSS ORACLE FOR FWBTC/FWETH");
  await deploy("SLOracle", {
    from: deployer,
    proxy: false,
    args: [uniFactory.address, FWBTC.address, FWETH.address],
    log: true,
  });
  ...
  if (poolAddress === NULL_ADDRESS) {
    await (
      await SLfactory.createPool(FWETH.address, FWBTC.address, SLOracle.address)
    ).wait();
  ...
  save("SLPoolFWBTCFWETH", {abi: SLPool.abi, address: poolAddress});
