...
async function main() {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (sender) {
      const jellyStoplossFinnanceContract = await ethers.getContract('JellyStoplossFinnance', sender);
      await waitFor(jellyStoplossFinnanceContract.setMessage(messages[i]));
    }
  }
}
